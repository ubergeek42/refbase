<?php
	...
	function modsCollection($result)
	{
		...
		// Right now, individual records are objects and collections of records are strings
		...
		{
			// Define an array of search & replace actions (to ensure that special characters are encoded with their entities):
			...
			foreach ($row as $rowFieldName => $rowFieldValue)
				$row[$rowFieldName] = searchReplaceText($exportSearchReplacePatterns, $row[$rowFieldName]); // function 'searchReplaceText()' is defined in 'include.inc.php'

			// Note: except from the above conversion of angle brackets (i.e., '<' and '>') and ampersands ('&'), data will be exported as fetched from the MySQL database, i.e., there's NO conversion of:
			//       - higher ASCII chars
			//       - "human readable markup" that's used within plain text fields of the database to define rich text characters like italics, etc. (see '$markupSearchReplacePatterns' in 'ini.inc.php')


			$record = modsRecord($row); // Export the current record as MODS XML

			if (!empty($record)) // unless the record buffer is empty...
				array_push($exportArray,$record); // ...add the current record to an array of exports
		}
