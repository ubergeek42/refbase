<?php
	...
	// File:       ./citation_style_1.inc.php
	// Created:    28-Sep-04, 22:14
	// Modified:   28-Sep-04, 23:39

	// This is a citation style file (which must reside within the 'styles' directory of your refbase root directory). It contains a
		...
		elseif ($row['type'] == "Book Chapter")
			{
				...
				if (!empty($row['abbrev_series_title'])) // if there's an abbreviated series title, series information will replace the publisher & place information
					{
		...
		elseif (ereg("Book Whole|Map|Manuscript|Journal", $row['type']))
			{
				...
				if (!empty($row['place']))			// place
					{
						$record .= $row['place'];
						if (!empty($row['abbrev_series_title']) || !empty($row['pages']))
							{
								if (!ereg(",$", $row['place']))
									$record .= ",";
								$record .= " ";
				...
				if (!empty($row['abbrev_series_title']))	// abbreviated series title
					{
						...
						// series volume & series issue will get appended only if there's also an abbreviated series title(!):
						if (!empty($row['series_volume'])||!empty($row['series_issue']))
							$record .= " ";
